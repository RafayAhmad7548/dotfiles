...
struct keyinfo{
    ...
    long long time;
    Hyprlang::INT* delay;
    Hyprlang::STRING* cmd_single;
    Hyprlang::STRING* cmd_double;
...
void thread_func(keyinfo& info){
    ...
    if(info.pressedOnce){
        ...
        if(strlen(*info.cmd_single) != 0) system(*info.cmd_single);
        info.pressedOnce = false;
    }
}

void detect_double_press(keyinfo& info){
    static auto* const ESC_INFO = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:esc_delay")->getDataStaticPtr();
    std::cout<<**ESC_INFO<<std::endl;
    if(!info.pressedOnce){
        ...
        if(current_time() - info.time <= *info.delay){
            ...
            if(strlen(*info.cmd_double) != 0) system(*info.cmd_double);
        }
        info.pressedOnce = false;
    }
}
...
void handleKey(struct wl_listener *listener, void *data){
    ...
    if(once){
        once = false;
        esc_info.delay = (Hyprlang::INT*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:esc_delay")->getDataStaticPtr();
        // shift_info.delay = (Hyprlang::INT*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:shift_delay")->getDataStaticPtr();
        // ctrl_info.delay = (Hyprlang::INT*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:ctrl_delay")->getDataStaticPtr();
        // alt_info.delay = (Hyprlang::INT*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:alt_delay")->getDataStaticPtr();
        // super_info.delay = (Hyprlang::INT*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:super_delay")->getDataStaticPtr();

        // esc_info.cmd_single = (Hyprlang::STRING*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:esc_single")->getDataStaticPtr();
        // shift_info.cmd_single = (Hyprlang::STRING*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:shift_single")->getDataStaticPtr();
        // ctrl_info.cmd_single = (Hyprlang::STRING*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:ctrl_single")->getDataStaticPtr();
        // alt_info.cmd_single = (Hyprlang::STRING*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:alt_single")->getDataStaticPtr();
        // super_info.cmd_single = (Hyprlang::STRING*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:super_single")->getDataStaticPtr();

        // esc_info.cmd_double = (Hyprlang::STRING*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:esc_double")->getDataStaticPtr();
        // shift_info.cmd_double = (Hyprlang::STRING*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:shift_double")->getDataStaticPtr();
        // ctrl_info.cmd_double = (Hyprlang::STRING*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:ctrl_double")->getDataStaticPtr();
        // alt_info.cmd_double = (Hyprlang::STRING*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:alt_double")->getDataStaticPtr();
        // super_info.cmd_double = (Hyprlang::STRING*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:morebinds:super_double")->getDataStaticPtr();
        std::cout<<*esc_info.delay<<std::endl;
    }
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle){
    ...
    std::cout<<HyprlandAPI::addConfigValue(PHANDLE, "plugin:morebinds:esc_delay", Hyprlang::INT{200})<<std::endl;
