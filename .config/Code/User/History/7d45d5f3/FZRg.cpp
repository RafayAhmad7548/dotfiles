...
bool once = true;
std::function<dispatchExec(std::string)>;
...
void thread_func(keyinfo& info){
    ...
    if(info.pressedOnce){
        ...
        if(strlen(*info.cmd_single) != 0) g_pKeybindManager->m_mDispatchers.at("exec")(*info.cmd_single);
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle){
    ...
    execDispatch = g_pKeybindManager->m_mDispatchers.at("exec");
